...
class Game extends React.Component {
              ...
              .then(res => {
                ...
                switch(turn) {
                  case '0':
                    ...
                    fetch('http://localhost:9000/ingame') // wait for other to join
                      .then(res => res.text())
                      .then(res => {
                    ...
                    break;
                  case '1': 
                    console.log('The opponent plays again')
                    this.setState({ showStart: true})
                    break;
                  case '2':
                    console.log('The host start the game')
                    fetch('http://localhost:9000/ingame')
                      .then(res => res.text())
                      .then(res => {
  ...
  componentDidMount() {
    ...
    if (this.state.turn === '1') { // the one create the game or host
      ...
      // Receive: (<player2>)
      fetch('http://localhost:9000/ingame')  // wait for an opponent and show the start box
        .then(res => res.text())
        .then(res => {
          this.setState({ player2: res, showStart: true})
        }).catch(err => console.error(err))
