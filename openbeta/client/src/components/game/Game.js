...
class Game extends React.Component {
  ...
  startGame() {
    ...
    axios.post('http://localhost:9000/start', req)
      .then(res => {
        ...
        if (this.state.playWithBot) {
          this.setState({board: board, currentTurn: turn, score1: tmp[65], score2: tmp[66], showStart: false})
          if (this.state.turn === '2') this.getBotMove()
        } else {
  ...
  getBotMove() {
    const req = { message: '5', roomID: '0', move_row: '8', move_col: '8'}

    axios.post('http://localhost:9000/getBotMove', req)
    .then(res => {
      const tmp = res.data.response.split(' ')
      const board = tmp.slice(0,64)
      const turn = tmp[64]
      ...
      this.setState({ board: board, currentTurn: turn, score1: score1, score2: score2})
      switch(turn) {
        case '-1': // the game finished
          console.log('[GAME FINISHED]')
          console.log(score1 + ' ' +  score2)
          ...
          break;
        case '1': case '2': // the component keeps playing
          if (turn !== this.state.turn) this.getBotMove()
          break;
        default: break;
      }
    }).catch(err => console.error(err))
  ...
  spectatorQuit() {
    ...
    axios.post('http://localhost:9000/spectateQuit', req)
      .then(res => {
  ...
  quitGame() {
    ...
    if (this.state.playWithBot) {
      ...
      if (this.state.turn === '3') {
        ...
        axios.post('http://localhost:9000/quit', req)
          .then(res => {
            if (res.data.response === '0') {
              ReactDOM.render(<Multi username={this.props.username} />, document.getElementById('root'))
            }
  ...
  quitBotGame() {
    const req = {message: '9 0'}

    axios.post('http://localhost:9000/quitBotGame', req)
    .then(res => {
      if (res.data.response === '0') {
        ReactDOM.render(<Single username={this.props.username} />, document.getElementById('root'));
      }
    }).catch(err => console.error(err))
  }
  ...
  playAgain() {
    ...
    axios.post('http://localhost:9000/playAgain', req)
      .then(res => {
        const tmp = res.data.response.split(' ')
        ...
        switch(turn) {
          ...
          case '3':
            ...
            break
          case '4':
            this.setState({ showStart: true, turn: tmp[65] }) 
  ...
  handleClick = (i) => {
    ...
    axios.post('http://localhost:9000/move', req)
    .then(res => {
      const tmp = res.data.response.split(' ')
      const board = tmp.slice(0,64)
      const turn = tmp[64]
  ...
  componentDidMount() {
    ...
    switch(this.state.turn) {
      case '0':
        this.setState({ showStart: true, turn: '1', playWithBot: true })
        break
